...
#define COM_SIZE CGSizeMake(266, 156)//通讯框大小
#define COM_TEXT @"加载中"//通讯框文字
#define COM_TYPE_CUSTOM //如果注释，则通讯框loading圈显示为自定义，否则显示默认样式
...
+ (MBProgressHUD *) getInstance
{
    if (!HUD)
    {
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
            ...
            HUD.minSize = COM_SIZE;
        }
        HUD.labelText = COM_TEXT;
...
- (void)dismissIndicatorWithType:(NSInteger)type andMessage:(NSString*)str{
    if (type == typeYes) {
        UIImage *image = [UIImage imageNamed:@"YES"];
        UIImageView *imageView = [[UIImageView alloc] initWithImage:image];
        HUD.customView = MB_AUTORELEASE(imageView);
        HUD.mode = MBProgressHUDModeCustomView;
    }else{
        UIImage *image = [UIImage imageNamed:@"NO"];
        UIImageView *imageView = [[UIImageView alloc] initWithImage:image];
        HUD.customView = MB_AUTORELEASE(imageView);;
        HUD.mode = MBProgressHUDModeCustomView;
    }
