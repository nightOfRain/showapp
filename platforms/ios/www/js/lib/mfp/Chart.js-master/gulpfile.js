...
gulp.task('build', function(){
	...
	return gulp.src(srcFiles)
		...
		.pipe(replace('{{ version }}', package2.version))
...
gulp.task('bump', function(complete){
	util.log('Current version:', util.colors.cyan(package2.version));
	var choices = ['major', 'premajor', 'minor', 'preminor', 'patch', 'prepatch', 'prerelease'].map(function(versionType){
		return versionType + ' (v' + semver.inc(package2.version, versionType) + ')';
	});
	inquirer.prompt({
		type: 'list',
		name: 'version',
		...
		var increment = res.version.split(' ')[0],
			newVersion = semver.inc(package2.version, increment);

		// Set the new versions into the bower/package object
		package2.version = newVersion;
		bower.version = newVersion;

		// Write these to their own files, then build the output
		fs.writeFileSync('package.json', JSON.stringify(package2, null, 2));
...
gulp.task('release', ['build'], function(){
	exec('git tag -a v' + package2.version);
});
