...
function pushPage(pid,method){
	if(pid.indexOf(".")==-1){
		if(!$("body").hasClass("edit")){
			zIndex++;
			var elme=$(".container-fluid[page='"+pid+"']");
			var pushPageEnd=function(){
				elme[0].removeEventListener("webkitAnimationEnd", pushPageEnd, false);
				elme.removeClass(method);
			};
			elme[0].addEventListener("webkitAnimationEnd", pushPageEnd, false);
			elme.css("z-index",zIndex).show().addClass(method);
		}
	}else{
		if($("#mainFrame5").attr("src") != pid) {
			$("#mainFrame5").attr("src", pid);
		}
		if($("#mainFrame5").attr("src")!=""){
...
function popPage(pid,method){
	if(pid.indexOf(".")==-1){
		if(!$("body").hasClass("edit")){
			var elme=$(".container-fluid[page='"+pid+"']");
			var popPageEnd=function(){
				elme[0].removeEventListener("webkitAnimationEnd", popPageEnd, false);
				if(!$("body").hasClass("sourcepreview"))elme.hide();
				elme.removeClass(method);
			};
			elme[0].addEventListener("webkitAnimationEnd", popPageEnd, false);
			elme.addClass(method);
		}
	}else{
		var hideframeback=function(){
			$('#newWin',window.parent.document)[0].removeEventListener("webkitAnimationEnd", hideframeback, false);
			$('#newWin',window.parent.document).hide().removeClass(method);
		};
		$('#newWin',window.parent.document)[0].addEventListener("webkitAnimationEnd", hideframeback, false);
		$('#newWin',window.parent.document).addClass(method);
	}
}
...
function zlshowLoading(){
	$(".loadingbar1").show().css("display","-webkit-box").addClass("tanpop");
}

function zlhideLoading(){
	$(".loadingbar1").hide();
}


function zlshowInputPop(){
	$(".inputPop").show().addClass("tanpop");
}

function zlhideInputPop(){
	$(".inputPop").hide();
}


function zlshowptPop(){
	$(".ptPop").show().addClass("tanpop");
}

function zlhideptPop(){
	$(".ptPop").hide();
}
